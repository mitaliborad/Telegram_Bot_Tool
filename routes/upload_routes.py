...
# gdrive_upload_executor = ThreadPoolExecutor(max_workers=2, thread_name_prefix='GDriveUpload')
...
# def stream_file_to_batch():
#     """
#     Receives a file from the client, saves it to a temp disk file,
#     and immediately starts a background job to upload it to GDrive.
#     """
#     if request.method == 'OPTIONS':
#         return make_response(("OK", 200))
...
#     batch_id = request.args.get('batch_id')
#     if not batch_id:
#         return jsonify({"error": "Missing 'batch_id' query parameter."}), 400
...
#     try:
#         # Save incoming stream to a temporary file on disk. This is fast.
#         with tempfile.NamedTemporaryFile(delete=False, dir=UPLOADS_TEMP_DIR, prefix=f"gdrive_up_{batch_id}_") as tf:
#             temp_file_path = tf.name
#             shutil.copyfileobj(request.stream, tf)
        
#         logging.info(f"{log_prefix} File '{filename}' saved to temp path: {temp_file_path}. Submitting to GDrive upload pool.")
        
#         # Submit the slow GDrive upload as a background task.
#         gdrive_upload_executor.submit(
#             run_gdrive_upload_and_update_db,
#             batch_id=batch_id,
#             temp_file_path=temp_file_path,
#             original_filename=filename
#         )
        
#         # Immediately return success to the frontend, allowing it to send the next file.
#         return jsonify({"message": f"File '{filename}' received and scheduled for upload."}), 202
            
#     except Exception as e:
#         logging.error(f"{log_prefix} Failed to receive and schedule file '{filename}': {e}", exc_info=True)
#         upload_progress_data[batch_id] = {"type": "error", "message": f"Server error receiving file {filename}."}
#         return jsonify({"error": "Server failed to process file stream."}), 500
...
# def run_gdrive_upload_and_update_db(batch_id: str, temp_file_path: str, original_filename: str):
#     """
#     A background task that uploads a file from a temp path to GDrive,
#     updates the database, and cleans up the temp file.
#     """
#     log_prefix = f"[BgGDriveUpload-{batch_id}]"
#     gdrive_file_id = None
    
#     try:
#         file_size = os.path.getsize(temp_file_path)
#         logging.info(f"{log_prefix} Starting GDrive upload for '{original_filename}' from '{temp_file_path}'.")
        
#         upload_generator = upload_to_gdrive_with_progress(
#             source=temp_file_path, 
#             filename_in_gdrive=original_filename, 
#             operation_id_for_log=batch_id
#         )
...
#             try:
#                 progress_event = next(upload_generator)
#                 progress_event['filename'] = original_filename
#                 upload_progress_data[batch_id] = progress_event
#             except StopIteration as e:
#                 gdrive_file_id, upload_error = e.value
#                 logging.info(f"{log_prefix} GDrive upload finished for '{original_filename}'. GDrive ID: {gdrive_file_id}, Error: {upload_error}")
#                 break
...
#         file_details_for_db = {
#             "original_filename": original_filename, "gdrive_file_id": gdrive_file_id,
#             "original_size": file_size, "mime_type": mimetypes.guess_type(original_filename)[0] or 'application/octet-stream',
#             "telegram_send_status": "pending"
#         }
#         coll, db_error = get_metadata_collection()
#         if db_error: raise Exception(f"DB connection error: {db_error}")
        
#         result = coll.update_one({"access_id": batch_id}, {"$push": {"files_in_batch": file_details_for_db}})
#         if result.matched_count == 0:
#             delete_from_gdrive(gdrive_file_id)
#             raise Exception(f"Batch ID '{batch_id}' not found in database for update.")
        
#         upload_progress_data[batch_id] = {"type": "status", "message": f"Completed: {original_filename}"}

#     except Exception as e:
#         if gdrive_file_id:
#             delete_from_gdrive(gdrive_file_id)
#         logging.error(f"{log_prefix} Background GDrive upload failed for '{original_filename}': {e}", exc_info=True)
#         upload_progress_data[batch_id] = {"type": "error", "message": f"Failed to upload {original_filename}: {e}"}
#     finally:
#         _safe_remove_file(temp_file_path, log_prefix, f"temp GDrive upload for {original_filename}")
...
#     try:
#         # Save to disk to avoid memory errors with large files
#         with tempfile.NamedTemporaryFile(delete=False, dir=UPLOADS_TEMP_DIR, prefix=f"gdrive_up_{batch_id}_") as tf:
#             temp_file_for_gdrive = tf.name
#             shutil.copyfileobj(request.stream, tf)
        
#         file_size = os.path.getsize(temp_file_for_gdrive)
#         logging.info(f"{log_prefix} File '{filename}' saved to temp path. Starting GDrive upload.")

#         # This is now a blocking, synchronous operation as per your requirement.
#         upload_generator = upload_to_gdrive_with_progress(
#             source=temp_file_for_gdrive, 
#             filename_in_gdrive=filename, 
#             operation_id_for_log=batch_id
...
#         logging.error(f"{log_prefix} Synchronous GDrive upload failed for '{filename}': {e}", exc_info=True)
#         upload_progress_data[batch_id] = {"type": "error", "message": str(e)}
#         return jsonify({"error": f"Failed to upload '{filename}': {str(e)}"}), 500
#     finally:
#         _safe_remove_file(temp_file_for_gdrive, log_prefix, "temp gdrive upload file")
...
#                         raw_results = [future.result() for future in as_completed(chunk_futures)]
#                         # Simplified result processing
#                         if all(res[1][0] for res in raw_results if res[0] == PRIMARY_TELEGRAM_CHAT_ID):
#                             updated_file_meta["telegram_send_status"] = "success_chunked"
#                         else:
#                             raise Exception("Primary Telegram chunked upload failed.")
...
def stream_file_to_batch():
    """
    Receives a file, saves it to disk, and then **synchronously** uploads to GDrive.
    This is a long-running request designed to wait until the GDrive upload is complete.
    """
    if request.method == 'OPTIONS':
        return make_response(("OK", 200))
    ...
    try:
        logging.info(f"{log_prefix} Receiving file '{filename}'.")
        # <<< FIX: Stream directly to a temporary file on disk to avoid memory errors >>>
        with tempfile.NamedTemporaryFile(delete=False, dir=UPLOADS_TEMP_DIR, prefix=f"gdrive_up_{batch_id}_") as tf:
            temp_file_for_gdrive = tf.name
            shutil.copyfileobj(request.stream, tf)
        
        file_size = os.path.getsize(temp_file_for_gdrive)
        logging.info(f"{log_prefix} File '{filename}' ({format_bytes(file_size)}) saved to temp path: {temp_file_for_gdrive}. Starting GDrive upload now.")

        # This is a blocking, synchronous operation as per your requirement. It will wait here.
        upload_generator = upload_to_gdrive_with_progress(
            source=temp_file_for_gdrive, 
            filename_in_gdrive=filename, 
            operation_id_for_log=batch_id
        ...
        # Consume the generator to drive the upload and send progress updates
        while True:
            try:
                progress_event = next(upload_generator)
                progress_event['filename'] = filename
                ...
                gdrive_file_id, upload_error = e.value
                logging.info(f"{log_prefix} GDrive upload for '{filename}' finished. GDrive ID: {gdrive_file_id}, Error: {upload_error}")
                break

        if upload_error or not gdrive_file_id:
            raise Exception(f"GDrive upload failed: {upload_error or 'No file ID returned'}")
    
        # Update the database after successful GDrive upload
        logging.info(f"{log_prefix} GDrive upload successful. Updating database for '{filename}'.")
        file_details = {
            "original_filename": filename, "gdrive_file_id": gdrive_file_id,
            "original_size": file_size, "mime_type": mimetypes.guess_type(filename)[0] or 'application/octet-stream',
            "telegram_send_status": "pending"
        ...
        result = coll.update_one({"access_id": batch_id}, {"$push": {"files_in_batch": file_details}})
        if result.matched_count == 0:
            delete_from_gdrive(gdrive_file_id) # Cleanup GDrive if DB record is gone
            raise Exception(f"Batch ID '{batch_id}' not found in database.")
        ...
        # <<< FIX: Always clean up the temp file from the server's disk >>>
        logging.info(f"{log_prefix} Cleaning up temporary file for '{filename}'.")
        _safe_remove_file(temp_file_for_gdrive, log_prefix, "temp gdrive upload file")

    logging.info(f"{log_prefix} Successfully processed '{filename}'. Sending 200 OK.")
    upload_progress_data[batch_id] = {"type": "status", "message": f"Completed: {filename}"}
    return jsonify({"message": f"File '{filename}' uploaded successfully to GDrive."}), 200
            ...
            for file_detail in files_to_process:
                ...
                try:
                    ...
                    if current_file_size > TELEGRAM_MAX_CHUNK_SIZE_BYTES:
                        ...
                        raw_results = [future.result() for future in as_completed(chunk_futures)]
                        if all(res[1][0] for res in raw_results if res[0] == str(PRIMARY_TELEGRAM_CHAT_ID)):
                            updated_file_meta["telegram_send_status"] = "success_chunked"
                        else:
                            raise Exception("Primary Telegram chunked upload failed.")
                    else:

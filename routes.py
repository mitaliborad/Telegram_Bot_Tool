...
# --- Flask Routes ---
# ... (index, initiate_upload, stream_progress routes remain unchanged) ...
@app.route('/')
def index() -> str:
...
# --- MODIFIED: _send_single_file_task accepts bytes ---
def _send_single_file_task(file_bytes: bytes, filename: str, chat_id: str, upload_id: str) -> Tuple[str, ApiResult]:
    """Task for sending a single file (from bytes) in a thread."""
    try:
        # Create a *new* buffer for this thread from the immutable bytes
        buffer = io.BytesIO(file_bytes)
        logging.info(f"[{upload_id}] T> Sending '{filename}' ({len(file_bytes)}b) to {chat_id}")
        # No need to seek, it's a new buffer
        result = send_file_to_telegram(buffer, filename, chat_id)
        buffer.close() # Close the thread-local buffer
        logging.info(f"[{upload_id}] T> Sent '{filename}' to {chat_id}. Success: {result[0]}")
        return str(chat_id), result
    except Exception as e:
        logging.error(f"[{upload_id}] T> Err send single file to {chat_id}: {e}", exc_info=True)
        return str(chat_id), (False, f"Thread error: {e}", None)

def _send_chunk_task(chunk_data: bytes, filename: str, chat_id: str, upload_id: str, chunk_num: int) -> Tuple[str, ApiResult]:
    """Task for sending a file chunk in a thread."""
    # (Unchanged - already uses bytes correctly)
    try:
...
def _download_chunk_task(file_id: str, part_num: int, prep_id: str) -> ChunkDataResult:
    """Task for downloading a file chunk in a thread."""
    # (Unchanged)
...
# ... (Helper functions: _parse_send_results, _calculate_progress, _find*, _calculate_download*, _safe_remove* remain unchanged) ...
...
def process_upload_and_generate_updates(upload_id: str) -> Generator[SseEvent, None, None]:
    """Generator handling the entire upload process and yielding SSE updates."""
    ...
    try:
        ...
        if total_size <= CHUNK_SIZE:
            ...
            # --- MODIFIED: Get bytes content ONCE ---
            file_bytes_content = zip_buffer.getvalue()
            zip_buffer.close() # Close the original buffer now
            ...
            if executor:
                for chat_id in TELEGRAM_CHAT_IDS:
                    cid = str(chat_id)
                    # --- MODIFIED: Pass file_bytes_content to task ---
                    fut = executor.submit(_send_single_file_task, file_bytes_content, comp_filename, cid, upload_id)
                    futures[fut] = cid
                logging.info(f"[{upload_id}] Submitted {len(futures)} single-file tasks.")
            else:
                cid = str(TELEGRAM_CHAT_IDS[0])
                # --- MODIFIED: Pass file_bytes_content to task ---
                _, res = _send_single_file_task(file_bytes_content, comp_filename, cid, upload_id)
                results[cid] = res

            # ... (rest of single file result processing remains the same) ...
            ...
            if success:
                ...
                record = {
                    "original_filename": original_filename, "sent_filename": comp_filename, "is_split": False, "is_compressed": True,
                    "original_size": total_size, "compressed_size": comp_size, "send_locations": details, "upload_timestamp": ts,
                    "username": username, "upload_duration_seconds": round(duration, 2), "access_id": access_id
                }
            ...
            # ... (Chunk sending loop remains the same - _send_chunk_task already uses bytes) ...
            # ... (Result processing and metadata saving remain the same) ...
            ...
            except Exception as e: logging.error(f"[{upload_id}] Err large compress: {e}", exc_info=True);
            ...
            temp_compressed_zip_filepath = None; raise
...
# --- Download Core Logic ---
# ... (_prepare_download_and_generate_updates remains unchanged from previous step) ...
def _prepare_download_and_generate_updates(prep_id: str) -> Generator[SseEvent, None, None]:
    ...
    try:
        ...
        if not is_split:
            ...
            speed = (dl_bytes/(1024*1024)/duration) if duration > 0 and dl_bytes > 0 else 0; logging.info(f"[{prep_id}] TG dl ({dl_bytes} bytes) in {duration:.2f}s. Speed:{speed:.2f}MB/s")
            ...
            if is_compressed:
             ...
             yield _yield_sse_event('status', {'message': f'Downloading {num_chunks} chunks...'})
             prog: Dict[str, Any] = {}
             for fut in as_completed(futures):
                try:
             ...
             if is_compressed:
                 ...
                 with tempfile.NamedTemporaryFile(delete=False, dir=UPLOADS_TEMP_DIR, prefix=f"dl_final_{prep_id}_") as tf:
                    ...
                    try:
                        ...
                        with zf.open(inner, 'r') as i, open(temp_final_file_path, 'wb') as o:
                            yield _yield_sse_event('progress', {'percentage': 90});
                            shutil.copyfileobj(i, o, length=DEFAULT_CHUNK_READ_SIZE)
                    finally:
                         if zf: zf.close()
                 temp_decompressed_path = temp_final_file_path
                 yield _yield_sse_event('progress', {'percentage': 98})
             else: temp_final_file_path = temp_reassembled_zip_path; temp_reassembled_zip_path = None; logging.info(f"[{prep_id}] Using reassembled directly."); yield _yield_sse_event('progress', {'percentage': 98})
        ...
        if temp_reassembled_zip_path and temp_reassembled_zip_path != temp_final_file_path: _safe_remove_file(temp_reassembled_zip_path, prep_id, "intermediate reass")
        if temp_decompressed_path and temp_decompressed_path != temp_final_file_path: _safe_remove_file(temp_decompressed_path, prep_id, "intermediate decomp")
...
def serve_temp_file(temp_id: str, filename: str) -> Response:
    """Serves the prepared temporary file and cleans it up."""
    logging.info(f"Request serve temp file ID: {temp_id}, Filename: {filename}")
    prep_info = download_prep_data.get(temp_id)
    if not prep_info: return make_response(f"Error: Invalid link (ID: {temp_id}).", 404)
    if prep_info.get('status') != 'ready':
        err = prep_info.get('error','Not ready.'); status = prep_info.get('status','unknown')
        logging.error(f"Serve fail: '{temp_id}' not ready. Status:{status}. Err:{err}")
        if temp_id in download_prep_data: del download_prep_data[temp_id]
        return make_response(f"Error: File not ready (Status:{status}, Err:{err}).", 400)
    temp_path = prep_info.get('final_temp_file_path'); size = prep_info.get('final_file_size')
    dl_name = prep_info.get('original_filename', filename)
    if not temp_path or not os.path.exists(temp_path):
        logging.error(f"Serve fail: Prepared file missing ID '{temp_id}'. Path:{temp_path}")
        if temp_id in download_prep_data: prep_info['status']='error'; prep_info['error']='File missing'
        return make_response("Error: Prepared file missing.", 500)
    def generate_and_cleanup(path: str, pid: str):
        logging.debug(f"[{pid}] Streaming from: {path}")
        try:
            with open(path, 'rb') as f:
                while True:
                    ...
                    if not chunk: break;
                    yield chunk;
        except Exception as e: logging.error(f"[{pid}] Error streaming {path}: {e}", exc_info=True)
        finally:
            logging.info(f"[{pid}] Cleanup post-stream."); _safe_remove_file(path, pid, "final dl")
            if pid in download_prep_data:
                logging.debug(f"[{pid}] Removing prep data.");
                try: del download_prep_data[pid]
                except KeyError: pass
    logging.info(f"[{temp_id}] Preparing streaming response for '{dl_name}'.")
    response = Response(stream_with_context(generate_and_cleanup(temp_path, temp_id)), mimetype='application/octet-stream')
    try: enc_name = dl_name.encode('utf-8').decode('latin-1')
    except UnicodeEncodeError: enc_name = f"download_{temp_id}.dat"; logging.warning(f"[{temp_id}] Fallback filename: {enc_name}")
...
def list_user_files(username: str) -> Response:
    """Lists file records for a given username."""
...
def get_file_by_access_id(access_id: str) -> Union[str, Response]:
    """Renders the download landing page for a given access ID."""
...
def delete_file_record(username: str, filename: str) -> Response:
    """Deletes a file record from the metadata."""
